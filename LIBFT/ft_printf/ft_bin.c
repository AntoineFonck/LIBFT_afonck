...
int pad_bin_prec_min(int binlen, t_flags *flags, int fd, uintmax_t bin)
{
    ...
    nbpad = flags->field_width - (flags->precision >= binlen ? flags->precision : binlen)
	    - (flags->precision <= binlen ? (flags->state & HASHTAG) : 0);
    if (nbpad < 0)
        nbpad = 0;
    nbzero = (flags->precision >= binlen ? flags->precision : binlen) - binlen;
    padlen = nbpad + nbzero + ((flags->state & HASHTAG) && flags->precision <= binlen ? 1 : 0);
