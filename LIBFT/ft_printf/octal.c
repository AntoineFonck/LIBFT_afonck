...
int	pad_oct_prec_min(int octlen, t_flags *flags, int fd, uintmax_t oct)
{
	...
	nbpad = flags->field_width - (flags->precision >= octlen ? flags->precision : octlen) - (flags->precision <= octlen ? (HASH_FLAG) : 0);
	if (nbpad < 0)
		nbpad = 0;
	nbzero = (flags->precision >= octlen ? flags->precision : octlen) - octlen;
	padlen = nbpad + nbzero + ((HASH_FLAG) && flags->precision <= octlen ? 1 : 0);
