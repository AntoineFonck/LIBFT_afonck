...
int pad_float_prec(double number, t_flags *flags, int fd)
{
	int nbpad;
	int nbzero;
	int padlen;
...
int float_precision(double number, int fd, t_flags *flags)
{
	int len;
	...
	if (!(flags->state & MINUS))
	{
		len += pad_float_prec(number, flags, fd);
...
int float_no_precision(double number, int fd, t_flags *flags)
{
	int len;
...
int special_convert_float(double number, int fd, t_flags *flags)
{
	...
	full_len = 0;
	if (flags->precision)
		full_len += float_precision(number, fd, flags);
	else
		full_len += float_no_precision(number, fd, flags);
